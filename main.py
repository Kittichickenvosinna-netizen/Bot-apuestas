...
'):
        ...
        if any(kw in norm_line for kw in keywords):
            if ':' in line:
                return line.split(':', 1)[1].strip()
    # Fallback: ultimas 2 lineas no vacias
...
def parse_prediccion(texto: str, mercado_default: str):
    ...
    # Extraer PREDICCION (flexible: PREDICCION, PREDICCION, PREDICTION, MEJOR VALOR)
    ...
    if pred:
        ...
        # Extraer linea (numero con punto o entero)
    ...
    # Extraer VALOR (flexible: VALOR, VALUE, APUESTA, RECOMENDACION)
    ...
    # Si no hay valor pero si prediccion, usar prediccion como valor
    if not resultado["valor"] and resultado["prediccion"]:
        resultado["valor"] = resultado["prediccion"]

    # Extraer STAKE (flexible: STAKE, CONFIANZA, UNIDADES, UNITS)
    ...
    # Extraer CUOTA explicita si no se encontro en valor
...
"
        ...
        # Solo guardar y enviar si se parseo correctamente
...
"
        ...
        # Parseo especial para /predecir
        parsed = parse_prediccion(pred_text, "mix")

        # Si no encontro prediccion, buscar en MEJOR VALOR o 1X2
