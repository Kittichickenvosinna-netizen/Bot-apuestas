...
async def corners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m in selected:
        ...
        prompt = (
            "Eres experto en CORNERS en ligas MENORES. Analiza estos datos y responde de forma CONCISA (max 800 caracteres).\n\n"
            + data
            + "\n\nResponde en español con este formato exacto:\n"
            "PREDICCION: [Over/Under X.5 corners]\n"
            "VALOR: [apuesta con mas valor y cuota estimada]\n"
            "STAKE: [1-10]\n"
            "POR QUE: [2-3 lineas maximo explicando el error del bookmaker]"
        )
...
async def goles(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m in selected:
        ...
        prompt = (
            "Eres experto en OVER/UNDER GOLES en ligas MENORES. Analiza estos datos y responde de forma CONCISA (max 800 caracteres).\n\n"
            + data
            + "\n\nResponde en español con este formato exacto:\n"
...
async def predecir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        prompt = (
            "Eres experto en VALOR en apuestas. Analiza estos datos y responde de forma CONCISA (max 1000 caracteres).\n\n"
            + data
            + "\n\nLIGA: " + lt
            + "\n\nResponde en español con este formato exacto:\n"
            "1X2: [prediccion]\n"
            "CORNERS: [Over/Under X.5]\n"
            "GOLES: [Over/Under X.5 + BTTS Si/No]\n"
            "MEJOR VALOR: [apuesta con mas valor y cuota justa estimada]\n"
            "STAKE: [1-10]\n"
            "RIESGO: [1 linea maximo]"
        )
    else:
        prompt = (
            "Analiza " + e1 + " vs " + e2
