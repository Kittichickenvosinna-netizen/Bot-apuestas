...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "🤖 Bot de Apuestas - Versión Simplificada\n\n"
        "📋 COMANDOS:\n"
        ...
        "💡 Todo se guarda y actualiza automático"
    )
    ...
    if len(context.args) == 1:
        try:
            ...
            if 1 <= valor <= 10:
                ...
                await update.message.reply_text(f"✅ Filtro: solo picks con stake ≥ {valor}")
            else:
                await update.message.reply_text("❌ El stake debe ser entre 1 y 10")
        except:
            await update.message.reply_text("❌ Uso: /stake 7")
    else:
        await update.message.reply_text(f"⚙️ Stake mínimo actual: {USER_CONFIG['min_stake']}\nUso: /stake 7")
    ...
    if len(context.args) == 1:
        try:
            ...
            if len(partes) == 2:
                ...
                await update.message.reply_text(f"✅ Filtro de cuotas: {min_odds} - {max_odds}")
            else:
        ...
        await update.message.reply_text(
            f"⚙️ Rango de cuotas actual: {USER_CONFIG['min_odds']} - {USER_CONFIG['max_odds']}\n"
            f"Uso: /odds 1.5,3.0"
        )

async def hoy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Muestra partidos + picks de goles y corners de una vez"""
    ...
    # Tomar hasta 3 partidos (más variados)
    selected = matches[:3]
    picks_goles = []
    picks_corners = []
    ...
    for m in selected:
        ...
        # === GOLES ===
        prompt_goles = (
            f"{ug_tag} Eres experto en OVER/UNDER GOLES. Analiza y responde EXACTAMENTE en este formato (max 500 chars):\n\n"
            + data
            + "\n\nFORMATO OBLIGATORIO:\n"
        ...
        if cumple_g:
            picks_goles.append(f"{ug_tag} {parsed_g['valor']} | {h} vs {a} | Stake {parsed_g['stake']}")

        # === CORNERS ===
        prompt_corners = (
            f"{ug_tag} Eres experto en CORNERS. Analiza y responde EXACTAMENTE en este formato (max 500 chars):\n\n"
            + data
            + "\n\nFORMATO OBLIGATORIO:\n"
        ...
        if cumple_c:
            picks_corners.append(f"{ug_tag} {parsed_c['valor']} | {h} vs {a} | Stake {parsed_c['stake']}")

    # === ARMAR RESPUESTA ===
    msg_lines = [f"📅 PICKS DEL DÍA - {datetime.now().strftime('%d/%m')}\n"]

    if picks_goles:
        msg_lines.append("⚽ GOLES:")
        for p in picks_goles:
            msg_lines.append(f"   {p}")
        msg_lines.append("")

    if picks_corners:
        msg_lines.append("🚩 CORNERS:")
        for p in picks_corners:
            msg_lines.append(f"   {p}")
        msg_lines.append("")

    if not picks_goles and not picks_corners:
        msg_lines.append("📊 Ningún pick cumple tus filtros actuales.")
        msg_lines.append(f"   Stake ≥ {min_stake} | Cuotas {min_odds}-{max_odds}")
        msg_lines.append("   Usa /stake 0 y /odds 0,999 para ver todos.")
    else:
        msg_lines.append(f"💡 Filtros: stake ≥ {min_stake} | Cuotas {min_odds}-{max_odds}")

    msg_lines.append("🟢 = Liga menor | 🔴 = Liga top")

    await update.message.reply_text("\n".join(msg_lines))

async def live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Muestra partidos en vivo + picks live de una vez"""
    ...
    # Filtrar ventana óptima (30-75 min)
    ...
    for m in selected:
        ...
        if cumple:
            picks_live.append(f"{ug_tag} {parsed['valor']} | {h} {hg}-{ag} {a} ({mn}\') | Stake {parsed['stake']}")
    ...
    if picks_live:
        ...
        msg_lines.append("📊 Ningún live pick cumple tus filtros.")
        msg_lines.append(f"   Stake ≥ {min_stake} | Cuotas {min_odds}-{max_odds}")
...
async def predecir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        prompt = (
            f"{ug_tag} | Eres experto en VALOR. Analiza y responde EXACTAMENTE en este formato (max 700 chars):\n\n"
            + data
            + "\n\nFORMATO OBLIGATORIO:\n"
            "1X2: [prediccion]\n"
