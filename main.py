...
def get_matches(date_str: str = None, underground_only: bool = False):
    ...
    # Solo partidos que aún no han empezado (pre-partido)
    matches = [m for m in matches if m.get("fixture", {}).get("status", {}).get("short", "") in ("NS", "TBD")]
    if underground_only:
...
def parse_prediccion(texto: str, mercado_default: str):
    ...
    for line in lines:
        ...
        if line.upper().startswith("PREDICCION:"):
            ...
            punt_match = re.search(r'(\d+)[/\s]*10', line)
            if punt_match:
                try:
...
async def hoy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m in valid_matches:
        ...
        if cumple_g and parsed_g["stake"] >= 5 and "NO HAY VALOR" not in pred_goles.upper():
            analisis_g = ""
            if "POR QUE:" in pred_goles:
                analisis_g = pred_goles.split("POR QUE:")[-1].strip()
        ...
        if cumple_c and parsed_c["stake"] >= 5 and "NO HAY VALOR" not in pred_corners.upper():
            analisis_c = ""
            if "POR QUE:" in pred_corners:
                analisis_c = pred_corners.split("POR QUE:")[-1].strip()
    ...
    if picks_enviados == 0:
        await update.message.reply_text(
            ...
            f"Stake ≥ {min_stake} (mínimo recomendado: 5) | Cuotas {min_odds}-{max_odds}\n"
...
async def live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m in selected:
        ...
        prompt = (
            ...
            f"REGLAS: total <5 → 'NO HAY VALOR'. Stake 5-6 (5-6 pts), 7-8 (7-8 pts), 9-10 (9-10 pts).\n\n"
            f"FORMATO OBLIGATORIO:\n"
        ...
        if cumple and parsed["stake"] >= 5 and "NO HAY VALOR" not in pred_text.upper():
            analisis = ""
            if "POR QUE:" in pred_text:
                analisis = pred_text.split("POR QUE:")[-1].strip()
...
async def predecir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    today = get_today_str()
    tomorrow = (datetime.now(ZoneInfo(USER_TIMEZONE)) + timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    if match:
        ...
        prompt = (
            ...
            f"REGLAS: total <5 → 'NO HAY VALOR' para ese mercado.\n\n"
            f"FORMATO OBLIGATORIO:\n"
            f"1X2: [prediccion]\n"
        ...
        await update.message.reply_text(f"❌ No encontré {e1} vs {e2} en las próximas 48 horas.")
